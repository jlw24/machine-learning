        ...
        *** THIS IS A BASE CLASS: YOU DO NOT NEED TO IMPLEMENT THIS ***
        ...
        Arguments:
            X - (np.array) An Nx(d+1) array of features, where N is the number
                ...
                the bias term.
            w - (np.array) A 1D array of parameters of length d+1. The gradientent
...
class SquaredLoss(Loss):
    ...
    def forward(self, X, w, y):
        ...
        for i in range(X.shape[0]):
            loss = np.append(loss, (y[i] - np.dot(np.transpose(w), X[i]))**2)

        loss = 0.5 * np.average(loss)
        """

        loss = 0.5 * np.average((y - np.sum((w * X), axis=1)) ** 2)
    ...
    def backward(self, X, w, y):
        ...
        """
        gradient = - np.average((y - np.sum((w * X), axis=1)) * np.transpose(X), axis=1)
...
class HingeLoss(Loss):
    ...
    def forward(self, X, w, y):
        ...
        # loss = np.clip(1 - y * np.sum((w * X), axis=1), 0, None)
        #
        # loss = np.array([])
        #
        # for i in range(X.shape[0]):
        #     z = np.dot(np.transpose(w), X[i])
        #     loss = np.append(loss, max(0, 1 - (y[i] * z)))
        #
        # loss = np.average(loss)

        loss = np.average(np.clip(1 - y * np.sum((w * X), axis=1), 0, None))
    ...
    def backward(self, X, w, y):
        ...
        for i in range(X.shape[0]):
            if 1 - (y[i] * np.dot(np.transpose(w), X[i])) > 0:
                num = (-y[i]*X[i])
            else:
                num = np.zeros((1, X.shape[1]))
            gradient = np.append(gradient, num)
