...
class GMM():
    ...
    def _e_step(self, features):
        ...
        for k in range(self.means.shape[0]):
            each = self._posterior(features, k)
            assign = np.append(assign, each)

        assign = assign.reshape((self.means.shape[0], -1))

        return np.transpose(assign)
    ...
    def _m_step(self, features, assignments):
        ...
        """
        N = features.shape[0]
        gam = np.sum(assignments, axis=0)

        newweight = gam/N
        newmeans = np.array([i for i in self.means])
        newcov = np.array([i for i in self.covariances])

        for i in range(newmeans.shape[0]):
            smallgam = assignments[:, i]
            smallgam = np.reshape(smallgam, (-1, 1))

            newmeans[i] = np.sum(smallgam*features, axis=0)/np.sum(smallgam)
            testcov = np.sum(smallgam*(features-self.means[i])**2, axis=0)/gam[i]
        ...
        self.means = newmeans
        self.covariances = newcov
        self.mixing_weights = newweight

        # for i in range(self.means.shape[0]):
        #     smallgam = np.transpose(np.array([assignments[:, i]]))
        #     feat = features
        #     gamfeat = smallgam*feat
        #     self.means[i] = np.sum(gamfeat)/np.sum(assignments[:, i])
        #     self.covariances = np.sum(smallgam * (features - self.means[i]**2))/np.sum(assignments[:, i])
    ...
    def _log_likelihood(self, features, k_idx):
        ...
        Returns:
            ...
            2 lines to extract mean and covariance from the specific cluster k_idx.

Then one line where you essentially use "multivariate_normal.logpdf"

and then you return the sum of the last line with the logged mixing_weights.

Remember, mean, covariance and mixing weights are members of the GMM class, so you can access them using self :)


        """

        LLmultivar = multivariate_normal.logpdf(features, self.means[k_idx], self.covariances[k_idx])

        loglike = np.log(self.mixing_weights[k_idx]) + LLmultivar
